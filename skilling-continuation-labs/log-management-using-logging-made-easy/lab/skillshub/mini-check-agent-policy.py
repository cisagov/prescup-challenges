...
POLICY_NAME = "mini-challenge" # Name of the agent policy that should exist
EXPECTED_INTEGRATIONS = {"system", "auditd", "mysql"}  # Integrations that should be present in the policy
...
def main():
    ...
    if not custom_policy:
        print(f"Failure -- The '{POLICY_NAME}' agent policy was not found.")
        return
    ...
    if EXPECTED_INTEGRATIONS.issubset(found_integrations):
        print(f"Success -- The '{POLICY_NAME}' agent policy exists and all required integrations are present.")
    else:
        missing = EXPECTED_INTEGRATIONS - found_integrations
        print(f"Failure -- The following integration(s) are missing from your agent policy: {', '.join(missing)}")
