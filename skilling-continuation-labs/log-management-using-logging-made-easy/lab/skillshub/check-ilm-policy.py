...
POLICY_NAME = "log-lme" # Name of the ILM policy that should exist
...
def get_ilm_policy(policy_name):
    ...
    if response.status_code == 404:
        print(f"FAIL: ILM policy '{policy_name}' does not exist.")
        return None
...
def validate_ilm_policy(policy):
    ...
    if warm_days == 30 and delete_days == 90:
        print(" SUCCESS: ILM policy is valid: Warm phase = 30 days, Delete phase = 90 days.")
        # API call to delete "logs@custom" component template to prepare environment for mini-challenge
        requests.delete(
            f"{ELASTIC_URL}/_component_template/logs@custom",
            headers = {
                    "Authorization": f"ApiKey {API_KEY}",
                    "Content-Type": "application/json"
            },
        ...
        print("FAIL: ILM policy does not meet the required retention settings.")
