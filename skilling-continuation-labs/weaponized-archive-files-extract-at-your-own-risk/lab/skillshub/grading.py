...
import paramiko, json, sys, subprocess, time, re, os
...
def phase1(sub=None):
    results = dict()
    try:
        ssh_command1 = "sshpass -p 'tartans' ssh -o StrictHostKeyChecking=no user@10.5.5.100 \"unrar l ~/Desktop/hello.rar\""
        output = subprocess.check_output(ssh_command1, shell=True, text=True)
        if 'etc/init.d/hello.txt' not in output:
            results['GradingCheck1'] = "Failure -- hello.txt was not found at the expected path within hello.rar."
            return results
    except:
        results['GradingCheck1'] = "Failure -- Error running unrar command. Ensure hello.rar exists on the Desktop."
        return results


    ssh_command2 = "sshpass -p 'tartans' ssh -o StrictHostKeyChecking=no user@10.5.5.100 \"[ -f /etc/init.d/hello.txt ]\""
    file_exists = subprocess.call(ssh_command2, shell=True) == 0
    if file_exists:
        results['GradingCheck1'] = "Success -- hello.rar was created properly and you were able to extract to the /etc/init.d directory."
    else:
        results['GradingCheck1'] = "Failure -- hello.txt was not found in the /etc/init.d directory. Try extracting the archive again and make sure to use sudo."
    return results

def phase2(sub=None):
    results = dict()
    try:
        ssh_command3 = "sshpass -p 'tartans' ssh -o StrictHostKeyChecking=no user@10.5.5.100 \"systemctl is-active initd-watch.service\""
        output = subprocess.check_output(ssh_command3, shell=True, text=True).strip()
        if output != 'active':
            results['GradingCheck2'] = "Failure -- The initd-watch.service is not active."
            return results
    except:
        results['GradingCheck2'] = "Failure -- Error checking service status. Please try again."
        return results
    try:
        ssh_command4 = "sshpass -p 'tartans' ssh -o StrictHostKeyChecking=no user@10.5.5.100 \"cat /var/log/initd_monitor.log\""
        log_output = subprocess.check_output(ssh_command4, shell=True, text=True)
        lines = log_output.splitlines()
    except:
        results['GradingCheck2'] = "Failure -- No logs have been written to /var/log/initd_monitor.log file yet. Ensure the service is running and correct."
        return results

    now = time.time()
    found = False
    for line in lines:
        match = re.match(r'\[(.*?)\] New file detected: /etc/init.d/hello\.txt', line)
        if match:
            try:
                log_time = time.mktime(parser.parse(match.group(1)).timetuple())
                if now - log_time <= 86400:
                    found = True
                    break
            except:
                continue
    ...
    if found:
        results['GradingCheck2'] = "Success -- The initd_monitor.service identified the new file and wrote the alert to the log file."
    else:
        results['GradingCheck2'] = "Failure -- The log file exists, but no recent valid log entry was found."
    return results
