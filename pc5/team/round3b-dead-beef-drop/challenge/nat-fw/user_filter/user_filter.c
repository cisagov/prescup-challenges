...
uint32_t tcp_payload_verdict(const char *buf)
{
	char command[128] = "/usr/share/pc5_user_filter/verdict ";
	char c, *p;
	int i;
	uint32_t ret;

	for (i = 0, p = command + strlen(command); i < 32; i++, p++) {
		c = buf[i];
		*p = isprint(c) ? c : '.';
	}

	/* call userspace program/script to get verdict */
	ret = system(command) ? NF_DROP : NF_ACCEPT;

	fprintf(stderr, "%s <- %s\n",
		(ret == NF_ACCEPT) ? "accept" : "drop", command);
		...
		struct nfgenmsg *nfmsg, struct nfq_data *nfad, void *data)
{
	 ...
	 */
	if (p_len >= 84)
		verdict = tcp_payload_verdict((char *)p_data + 52);
