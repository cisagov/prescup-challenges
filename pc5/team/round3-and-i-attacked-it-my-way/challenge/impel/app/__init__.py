...
def create_app(config_class=Config):
    ...
    with app.app_context():
        ...
        @app.before_request
        def check_timeout():
            if (request.method == 'POST') and (globals.timeout['active'] == True):
                flash("Must wait for timeout to end in order to attempt login again.")

    app.add_url_rule("/login/status",view_func=get_timeout_status, endpoint="/login/status")
