...
client = ModbusTcpClient("10.2.2.105", port=502)

# Initialize RPM and increment values
rpm = 2000
increment = 10

while rpm > 0 and rpm < 4000:
    try:
        client.connect()
    ...
    # Sleep for 1 second
    time.sleep(1)
#upon reaching 4000 rpm, write ERROR ERROR ERROR to the holding register
client.connect()
client.write_register(0, 0)
client.close()
print("client 0 written")
# Print the flag to a file
with open('motor_complete.txt', 'w') as flag_file:
    flag_file.write('motor_shutdown_complete')
