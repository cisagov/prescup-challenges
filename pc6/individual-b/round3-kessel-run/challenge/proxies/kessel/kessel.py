...
class HTTPHandler:
    ...
    def linesGen(self):
        ...
        while attempt < self.maxAttempts:
            ...
            if NEW_LINE not in self.data and len(self.data) >= 128:
                raise BadData
            ...
            while NEW_LINE in self.data:
                line, self.data = self.data.split(NEW_LINE, 1)
                yield line
    ...
    def craftResponse(self, code, body, headers):
        # Line 1
        response = f"HTTP/1.1 {code} {self.code2Message(code)}{NEW_LINE}"
        # Add Headers
        for h in headers:
            response += h + f"{NEW_LINE}"
        # Add the body
        if isinstance(body, str) and body != "":
            response += f"Content-Length: {len(body) + len(NEW_LINE)}{NEW_LINE}"
            # Add content length and the body
            response += f"{NEW_LINE}{body}{NEW_LINE}"
        elif isinstance(body, list) and len(body) != 0:
            # Add transfer encoding and the body
            response += f"Transfer-Encoding: chunked{NEW_LINE}{NEW_LINE}"
            for b in body:
                response += f"{len(b)}{NEW_LINE}"
                response += f"{b}{NEW_LINE}"
            response += f"0{NEW_LINE}{NEW_LINE}"
        else:
    ...
    def craftRequest(self, method, path, body, headers):
        # Line 1
        request = f"{method} {path} HTTP/1.1{NEW_LINE}"
        # Host line should be our proxy 
        request += f"Host: {PROXY}:{PROXY_PORT}{NEW_LINE}"
        # Only one request to the proxy
        request += f"Connection: close{NEW_LINE}"  # Only want to make one request
        for h in headers:  # Custom headers
            request += h + f"{NEW_LINE}"
        if isinstance(body, str) and body != "":
            # Add content length and the body
            request += f"Content-Length: {len(body) + len(NEW_LINE)}{NEW_LINE}"
            request += f"{NEW_LINE}{body}{NEW_LINE}"
        elif isinstance(body, list) and len(body) != 0:
            # Add transfer encoding and the body
            request += f"Transfer-Encoding: chunked{NEW_LINE}{NEW_LINE}"
            for b in body:
                request += f"{len(b)}{NEW_LINE}"
                request += f"{b}{NEW_LINE}"
            request += f"0{NEW_LINE}{NEW_LINE}"
        else:
            # No body (for my oooown oooohaaaaaaaa)
            request += f"{NEW_LINE}"
        return request
        ...
        while True:
            ...
            if path == "/token":
                with open("./token.html", "r") as f:
                    responseBody = f.read()
            elif path == "/" or path == "/kessel":
                with open("./kessel.html", "r") as f:
                    responseBody = f.read()
