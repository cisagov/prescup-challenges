...
ALLOWED_URLS = ["/", "/maelstrom", "/token", "/maw"]

HOST_ALIASES = ["maelstrom.us", "maelstrom.us:8080"]
...
class HTTPHandler:
    ...
    def linesGen(self):
        ...
        while attempt < self.maxAttempts:
            ...
            if NEWLINE not in self.data and len(self.data) >= 128:
                raise BadData

            # If no new line after getChunk, wait and try again
            if NEWLINE not in self.data:
                attempt += 1
            else:
                attempt = 0 

            # Read out lines in buffer
            while NEWLINE in self.data:
                line, self.data = self.data.split(NEWLINE, 1)
                yield line
    ...
    def craftResponse(self, code, body, headers):
        # Line 1
        response = f"HTTP/1.1 {code} {self.code2Message(code)}{NEWLINE}"
        # Add Headers
        for h in headers:
            response += h + f"{NEWLINE}"
        # Add the body
        if isinstance(body, str) and body != "":
            response += f"Content-Length: {len(body) + len(NEWLINE)}{NEWLINE}"
            # Add content length and the body
            response += f"{NEWLINE}{body}{NEWLINE}"
        elif isinstance(body, list) and len(body) != 0:
            # Add transfer encoding and the body
            response += f"Transfer-Encoding: chunked{NEWLINE}{NEWLINE}"
            for b in body:
                response += f"{len(b)}{NEWLINE}"
                response += f"{b}{NEWLINE}"
            response += f"0{NEWLINE}{NEWLINE}"
        else:
    ...
    def craftRequest(self, method, path, body, headers):
        # Line 1
        request = f"{method} {path} HTTP/1.1{NEWLINE}"
        # Host line should be our proxy 
        request += f"Host: {PROXY}:{PROXY_PORT}{NEWLINE}"
        # Only one request to the proxy
        request += f"Connection: close{NEWLINE}"  # Only want to make one request
        for h in headers:  # Custom headers
            request += h + f"{NEWLINE}"
        if isinstance(body, str) and body != "":
            # Add content length and the body
            request += f"Content-Length: {len(body) + len(NEWLINE)}{NEWLINE}"
            request += f"{NEWLINE}{body}{NEWLINE}"
        elif isinstance(body, list) and len(body) != 0:
            # Add transfer encoding and the body
            request += f"Transfer-Encoding: chunked{NEWLINE}{NEWLINE}"
            for b in body:
                request += f"{len(b)}{NEWLINE}"
                request += f"{b}{NEWLINE}"
            request += f"0{NEWLINE}{NEWLINE}"
        else:
            # No body (for my oooown oooohaaaaaaaa)
            request += f"{NEWLINE}"
        return request
        ...
        while True:
            ...
            if path == "/token":
                with open("./token.html", "r") as f:
                    responseBody = f.read()
            elif path == "/" or path == "/maelstrom":
                with open("./maelstrom.html", "r") as f:
                    responseBody = f.read()
