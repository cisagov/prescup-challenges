...
HOST_ALIASES = ["maw.us", "maw.us:8080"]
...
class HTTPHandler:
    ...
    def linesGen(self):
        ...
        while attempt < self.maxAttempts:
            ...
            if LINE_SEP not in self.data and len(self.data) >= 128:
                raise BadData

            # If no new line after getChunk, wait and try again
            if LINE_SEP not in self.data:
                attempt += 1
            else:
                attempt = 0 

            # Read out lines in buffer
            while LINE_SEP in self.data:
                line, self.data = self.data.split(LINE_SEP, 1)
                yield line
    ...
    def craftResponse(self, code, body, headers):
        # Line 1
        response = f"HTTP/1.1 {code} {self.code2Message(code)}{LINE_SEP}"
        # Add Headers
        for h in headers:
            response += h + f"{LINE_SEP}"
        # Add the body
        if isinstance(body, str) and body != "":
            response += f"Content-Length: {len(body) + len(LINE_SEP)}{LINE_SEP}"
            # Add content length and the body
            response += f"{LINE_SEP}{body}{LINE_SEP}"
        elif isinstance(body, list) and len(body) != 0:
            # Add transfer encoding and the body
            response += f"Transfer-Encoding: chunked{LINE_SEP}{LINE_SEP}"
            for b in body:
                response += f"{len(b)}{LINE_SEP}"
                response += f"{b}{LINE_SEP}"
            response += f"0{LINE_SEP}{LINE_SEP}"
        else:
    ...
    def craftRequest(self, method, path, body, headers):
        # Line 1
        request = f"{method} {path} HTTP/1.1{LINE_SEP}"
        # Host line should be our proxy 
        request += f"Host: {PROXY}:{PROXY_PORT}{LINE_SEP}"
        # Only one request to the proxy
        request += f"Connection: close{LINE_SEP}"  # Only want to make one request
        for h in headers:  # Custom headers
            request += h + f"{LINE_SEP}"
        if isinstance(body, str) and body != "":
            # Add content length and the body
            request += f"Content-Length: {len(body) + len(LINE_SEP)}{LINE_SEP}"
            request += f"{LINE_SEP}{body}{LINE_SEP}"
        elif isinstance(body, list) and len(body) != 0:
            # Add transfer encoding and the body
            request += f"Transfer-Encoding: chunked{LINE_SEP}{LINE_SEP}"
            for b in body:
                request += f"{len(b)}{LINE_SEP}"
                request += f"{b}{LINE_SEP}"
            request += f"0{LINE_SEP}{LINE_SEP}"
        else:
            # No body (for my oooown oooohaaaaaaaa)
            request += f"{LINE_SEP}"
        return request
        ...
        while True:
            ...
            if path == "/token":
                with open("./token.html", "r") as f:
                    responseBody = f.read()
            elif path == "/" or path == "/maw":
                with open("./maw.html", "r") as f:
                    responseBody = f.read()
